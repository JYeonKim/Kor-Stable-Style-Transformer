...
class styletransfer(nn.Module):
    def __init__(self, drop_rate=0, gpu = True):
        ...
        # self.n_vocab = 50259
        # self.emb_dim = 256
        self.n_vocab = 51200 # kogpt2에 맞춰 수정
        self.emb_dim = 768 # kogpt2에 맞춰 수정
        self.nhead = 4
        self.num_layers = 3
        ...
        # self.PAD_IDX = 50258
        # self.EOS_IDX = 50256
        self.START_IDX = 51198 # kogpt2에 맞춰 수정
        self.PAD_IDX = 51199 # kogpt2에 맞춰 수정
        self.EOS_IDX = 51197 # kogpt2에 맞춰 수정
    ...
    def decoder(self, enc_out, dec_input, attribute):
        with torch.autograd.set_detect_anomaly(True):
            ...
            # 수정
            dec_out = self.transformer_decoder(style_dec_input, enc_out, tgt_mask=tgt_mask).clone() # (dec_len+2, batch, emb_dim)
            # 수정
    ...
    def generator(self, enc_out, gen_len, attribute):
        with torch.autograd.set_detect_anomaly(True):
            ...
            for i in range(gen_len):
                ...
                vocab_out = self.matrix_D(dec_out).clone() # (pre_gen_len, batch, n_vocab)
    ...
    def cls_loss(self, attributes, cls_out):
        with torch.autograd.set_detect_anomaly(True):
            ...
            if self.gpu == True:       
                ...
                return cls_loss
            # cls_loss_value = F.cross_entropy(cls_out, targets).clone()
            # if self.gpu == True:
            #     return cls_loss_value.cuda()
            # else:
            #     return cls_loss_value.cuda()

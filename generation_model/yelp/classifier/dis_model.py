...
#         self.fc_drop = nn.Dropout(drop_rate)
    ...
    def discriminator(self, token_idx):
        ...
        x2 = F.relu(self.conv2d_2(word_2d), inplace=False).squeeze(3) # bi-gram, (batch, channel_out, seq_len-1)
        x3 = F.relu(self.conv2d_3(word_2d), inplace=False).squeeze(3) # 3-gram, (batch, channel_out, seq_len-2)
        x4 = F.relu(self.conv2d_4(word_2d), inplace=False).squeeze(3) # 4-gram, (batch, channel_out, seq_len-3)
        x5 = F.relu(self.conv2d_5(word_2d), inplace=False).squeeze(3) # 5-gram, (batch, channel_out, seq_len-4)
    ...
    def gen_discriminator(self, gen_out):
        ...
        y = self.disc_fc(x).clone() # (batch, 2)
    ...
    def att_prob(self, token_idx, sentiment):
        ...
        # del_idx = torch.from_numpy(np.asarray(final_list)).unsqueeze(0)
